...
(function() {
  ...
  (function() {
    ...
    this.createSocket = function () {
      ...
      var createSocketFunc = exports.createSocket || function() {
        var ws = new WebSocket('ws://' + window.location.host); // (ael removed) , 'shiny');
  ...
  exports.initShiny = function() {
    ...
    function bindMultiInput(scope, selector, exclusiveValue) {
      $(scope).on('change input', selector, function() {
    ...
    // ael: don't just assume we're operating in the entire document
    var match = $('.shiny-top');
    var shinyTop = match.length ? match[0] : document;
    // Initialize all input objects in the document, before binding
    initializeInputs(shinyTop);
    ...
    bindMultiInput(shinyTop, 'input[type="checkbox"]', false);
    bindMultiInput(shinyTop, 'input[type="radio"]', true);
    var initialValues = _bindAll(shinyTop);
